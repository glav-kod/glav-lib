...
Это гарантирует, что при переименовании enum-значения компилятор выдаст ошибку, и строковый ключ всегда совпадает с тем, что лежит в БД.

Если SQL-строка `const`, замени на `var` с `@$"..."`, чтобы включить интерполяцию: интерполяция в константе невозможна, и без этой замены пример не скомпилируется.
