...
ci/                        Скрипты выпуска: version.sh, pack.sh, push.sh, release.sh
.github/workflows/         GitHub Actions: publish.yml — сборка, упаковка, публикация пакетов и релиз
```
...
Плейсхолдеры в сообщении пакета языка пишутся ровно так же, как в `_errors/*.errors.yaml`:
`{name:type}`, где тип может быть необязательным (`{name:type?}`) и может сопровождаться строкой
формата (`{name:type:format}`, где сама строка формата ограничена буквами и цифрами — `dd.MM.yyyy`
или `#,##0.00` не разберёт ни одна сторона). Грамматику разбирают два места — `ErrorMessageTemplate` в генераторе
ошибок и `MultiLangMessage.Format`, — и расходиться они не должны: перевод переносит плейсхолдер
из исходного сообщения ошибки посимвольно, поэтому placeholder, который не понимает одна из сторон,
останется в тексте ответа как есть.
...
Прежнего пайплайна TeamCity на Kotlin DSL в репозитории больше нет, как раньше не стало
сборки на Nuke.

Workflow запускается на каждый push в любую ветку и вручную (`workflow_dispatch`).
Упаковка идёт всегда, выкладка — только из основной ветки либо при ручном запуске:
сборка из рабочей ветки даёт предрелизную версию с суффиксом имени ветки, и выложить её
можно, запустив workflow руками. Признак выкладки вычисляется один раз в переменной
задания `PUBLISH`, и публикация с релизом включаются им вместе. Готовые `.nupkg` каждый
прогон сохраняет артефактом сборки `packages`, поэтому пакет можно забрать из GitHub,
ничего не публикуя.

Версию workflow считает отдельным шагом и передаёт дальше через окружение: её берут
и упаковка, и релиз. Считать её дважды нельзя — прогон, начатый под полночь, получил бы
в теге и в пакетах разные даты.
...
| `ci/release.sh` | Создаёт релиз GitHub с тегом `v<версия>` и прикладывает к нему `artifacts/*.nupkg` |
...
Каждая выкладка сопровождается релизом GitHub: его создаёт `ci/release.sh` командой
`gh release`. Тег `v<версия>` GitHub заводит сам на коммите прогона, к релизу
прикладываются готовые `.nupkg`, а версия с суффиксом ветки помечает релиз предрелизным,
чтобы он не подменял собой последний выпуск из основной ветки. Заданию для этого нужно
разрешение `contents: write`, а скрипту — переменная `GH_TOKEN`; локально хватает
установленного и авторизованного `gh`.

Релиз — способ забрать пакет, не подключая дополнительных источников NuGet. Раскладывать
пакеты ещё и в GitHub Packages ради этого не нужно: это отдельный фид, который каждому
потребителю пришлось бы прописывать в `nuget.config` вместе с токеном, потому что
анонимного чтения там нет даже у публичного репозитория.

Повторный прогон того же задания workflow сохраняет прежний `github.run_number`, а значит
и версию пакетов, поэтому к повтору терпимы оба шага: `dotnet nuget push` идёт
с `--skip-duplicate`, а `ci/release.sh` вместо создания релиза обновляет существующий.
...
`docker-compose.yml` поднимает пару PostgreSQL 18: мастер на `127.100.0.1:5432` и потоковую
