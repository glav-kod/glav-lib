...
собирает решение, упаковывает пакеты и раскладывает их по трём местам — nuget.org,
GitHub Packages и релиз GitHub. Прежнего пайплайна TeamCity на Kotlin DSL в репозитории
больше нет, как раньше не стало сборки на Nuke.
...
задания `PUBLISH`, и все три шага выкладки включаются им вместе. Готовые `.nupkg` каждый
...
| `ci/push.sh` | Публикует `artifacts/*.nupkg` командой `dotnet nuget push` в хранилище, заданное переменной `NUGET_SOURCE` |
...
### GitHub Packages и релизы

Кроме nuget.org те же пакеты уезжают в GitHub Packages владельца репозитория. Отдельного
скрипта для этого нет: `ci/push.sh` вызывается второй раз с другим хранилищем —
`https://nuget.pkg.github.com/<владелец>/index.json` — и с `GITHUB_TOKEN` задания вместо
ключа nuget.org, поэтому заданию нужно разрешение `packages: write`. Чтобы опубликованный
пакет привязался к репозиторию, а не повис на владельце сам по себе, `ci/pack.sh`
проставляет в пакете поле `Repository` (`RepositoryUrl` и `RepositoryType`); адрес
репозитория берётся из переменной `REPOSITORY_URL`.

Релиз создаёт `ci/release.sh` командой `gh release`: тег `v<версия>` GitHub заводит сам
на коммите прогона, к релизу прикладываются готовые `.nupkg`, а версия с суффиксом ветки
помечает релиз предрелизным, чтобы он не подменял собой последний выпуск из основной
ветки. Заданию для этого нужно разрешение `contents: write`, а скрипту — переменная
`GH_TOKEN`; локально хватает установленного и авторизованного `gh`.

Повторный прогон того же задания workflow сохраняет прежний `github.run_number`, а значит
и версию пакетов, поэтому оба шага выкладки к повтору терпимы: `dotnet nuget push` идёт
