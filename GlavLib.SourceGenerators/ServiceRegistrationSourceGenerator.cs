...
public sealed class ServiceRegistrationSourceGenerator : IIncrementalGenerator
{
    ...
    private const string DomainEventHandlerAttributeSyntax = "DomainEventHandler";
    private const string SingleInstanceAttributeSyntax = "SingleInstance";
    private const string TransientAttributeNameSyntax = "Transient";
    private const string AddServicesFromAttributeSyntax = "AddServicesFrom";

    private static readonly DiagnosticDescriptor UnhandledException =
        new("XX0003",
            "Unhandled exception occurred",
            "The ServiceRegistrationSourceGenerator caused an exception {0}: {1} {2}",
            "Error",
    ...
    private static bool IsTransformCandidate(SyntaxNode node)
    {
        ...
        foreach (var attribute in classDeclarationSyntax.AttributeLists.SelectMany(x => x.Attributes))
        {
            ...
            if (attributeName.Contains(DomainEventHandlerAttributeSyntax)
                || attributeName.Contains(SingleInstanceAttributeSyntax)
                || attributeName.Contains(TransientAttributeNameSyntax)
                || attributeName.Contains(AddServicesFromAttributeSyntax))
            {
                return true;
            }
        }
        ...
        foreach (var attributeData in classSymbol.GetAttributes())
        {
            ...
            if (attributeData.AttributeConstructor is not null)
            {
                for (var i = 0; i < attributeData.AttributeConstructor.Parameters.Length; i++)
                {
                    ...
                    if (argument.Kind != TypedConstantKind.Primitive || argument.Value is null)
                        continue;
    ...
    private struct TypeRegistration : IEquatable<TypeRegistration>
    {
        ...
        public static TypeRegistration AddServicesFromMethod(
        ...
        public static TypeRegistration Singleton(
                string fullClassName,
                string? asType = null,
                string? key = null
            )
        ...
        public static TypeRegistration Transient(
                string fullClassName,
                string? asType = null,
                string? key = null
            )
