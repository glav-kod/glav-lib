...
public partial class ServiceRegistrationSourceGeneratorTests
{
    public class SingleInstance
    {
        ...
        [Fact]
        public void It_should_correctly_consume_attribute_array_parameter()
        {
            //language=CSharp
            const string source = """
                namespace TestNamespace;

                public class SystemUserRolesAttribute : System.Attribute
                {
                    public SystemUserRolesAttribute(params string[] roles)
                    {
                    }
                }

                [SystemUserRoles("some-role")]
                public sealed class TestClass
                {
                }
                """;

            var sourceGenerator = new ServiceRegistrationSourceGenerator().AsSourceGenerator();
            
            var result = RunSourceGenerator(sourceGenerator: sourceGenerator,
                                            sourceCode: source);


            var errors = result.Diagnostics.Where(x => x.Severity is DiagnosticSeverity.Warning or DiagnosticSeverity.Error);
