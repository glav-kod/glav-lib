...
## Метки, которых пока нет

Канонического набора меток триажа (`docs/agents/triage-labels.md`) и меток
`wayfinder:*` в репозитории может не оказаться: метка заводится не заранее, а вместе
с первой задачей, которой она понадобилась. Скилл, которому метка нужна, создаёт её
сам: `gh label create <имя> --description "..."`. Отсутствие метки — не повод
пропускать шаг и не повод спрашивать разрешения.
