...
Строку блокируем режимом **`for no key update`**, а не `for update`. Это касается и обычной
блокировки, и разбора очереди через `skip locked`. Правило про PostgreSQL: блокировок уровня
строки у SQLite нет, и к запросам под неё оно не относится.

Оба режима конфликтуют сами с собой и одинаково разводят параллельных обработчиков — ради чего
блокировка и берётся. Разница в другом: `for update` вдобавок конфликтует с `for key share` —
блокировкой, которую PostgreSQL берёт на родительскую строку, когда проверяет внешний ключ
вставляемой дочерней. Пока транзакция держит `for update`, любая вставка строки, ссылающейся
на заблокированную, встаёт в очередь за ней, хотя к сути блокировки отношения не имеет. Отсюда
взаимная блокировка там, где её не ждут: транзакция держит `for update` на родительской строке
и внутри себя (или в соседней транзакции, которую ждёт) вставляет дочернюю строку с внешним
ключом на неё же.

```sql
-- ❌ BAD — блокирует и вставку дочерних строк по внешнему ключу
select t.id
  from public.users t
 where t.id = :userId
   for update
...
Замена ничего не ослабляет там, где блокировка нужна на самом деле: `for no key update`
по-прежнему несовместим и сам с собой, и с `delete`, и с изменением ключа заблокированной строки.

**Когда всё же нужен `for update`.** Единственный случай — когда блокировка должна не пускать
появление дочерних строк: например, транзакция проверяет, что на родителя никто не ссылается,
и удаляет его. Такое место обязано нести комментарий с объяснением, почему обычного режима
не хватает; без объяснения `for update` в код не проходит.

**NHibernate.** `LockMode.Upgrade` в `GetAsync`/`LockAsync` порождает `select ... for update`
и этому правилу не подчиняется: режим задаётся диалектом NHibernate, а не запросом. Если строку
нужно заблокировать именно `for no key update` — пиши блокировку явным SQL через Dapper.
